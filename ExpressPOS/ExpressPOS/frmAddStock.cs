                    ...
                    foreach (DataGridViewRow Row in ProductDataGridView.Rows)
                    {
                        if (Row.Cells[0].Value != null)
                        {
                            if ((bool)(Row.Cells[0].Value) == true)
                            {
                                ...
                                double  imported_qty;
                                try { imported_qty = clsCN.num_repl(ProductDataGridView.Rows[Row.Index].Cells["Column5"].Value.ToString()); }
                                catch { imported_qty = 0; }

                                double imported_qty_value;
                                try { imported_qty_value = clsCN.num_repl(ProductDataGridView.Rows[Row.Index].Cells["Column7"].Value.ToString()); }
                                catch { imported_qty_value = 0; }
                                ...
                                double total_unit = imported_qty + stock_qty;
                                double total_stock_value = stock_qty * stock_unit_cost;

                                double unit_cost = (imported_qty_value + total_stock_value) / total_unit;

                                clsCN.ExecuteSQLQuery(" UPDATE Product SET Quantity = '" + total_unit + "', CostPrice ='" + unit_cost + "'  WHERE PRODUCT_ID = '" + product_id + "' ");
                                clsCN.ExecuteSQLQuery(" INSERT INTO StockMovement (PRODUCT_ID, SUPP_ID , EntryDate, Quantity, TotalCost, Stock) VALUES ('" + product_id + "', '" + supplier_id + "', '" + dtpEntryDate.Value.Date.ToString("MM/dd/yyyy") + "', '" + imported_qty + "', '" + imported_qty_value + "', 'StockIn') ");
                            }
                        }
                    }
                    LoadData();
